...
console.info('SA・PAナビ build v0.20');
...
function stepIsHighway(st){
  ...
  if(cls.includes('motorway')||cls.includes('motorway_link'))return true;
...
function segmentSummary(rt,analysisOverride=null){
  const a=analysisOverride||analyzeRoute(rt);
  if(!a.hasStepData && !a.forcedByIC)return {text:'区間判定情報なし',transition:'',segments:[],analysis:a};
  if(a.forcedByIC){
    const pre=a.highwayStart?.km||0, end=a.highwayEnd?.km||pre;
    const total=(rt.distance||0)/1000;
    return {
      text:`一般道 0.0–${pre.toFixed(1)}km → 高速IC経由 ${pre.toFixed(1)}–${end.toFixed(1)}km → 一般道 ${end.toFixed(1)}–${total.toFixed(1)}km`,
      transition:`高速道路開始: ${a.highwayStart.label}（約${pre.toFixed(1)}km） ／ 高速道路終了: ${a.highwayEnd.label}（約${end.toFixed(1)}km）`,
      segments:[],analysis:a
    };
  }
  // 道路名が変わっても「一般道」「高速」をまとめる。これで数十個の細切れ表示を防ぐ。
  const compact=[];
  for(const s of a.segments){
    const isHighway=s.isHighway;
    const last=compact.at(-1);
    if(last && last.isHighway===isHighway) last.toKm=s.toKm;
    else compact.push({label:isHighway?'高速道路':'一般道',isHighway,fromKm:s.fromKm,toKm:s.toKm});
  }
  const text=compact.map(s=>`${s.label} ${s.fromKm.toFixed(1)}–${s.toKm.toFixed(1)}km`).join(' → ');
  let transition='';
...
function classifyRoute(rt,i,kind='auto',meta={}){
  const a=meta.analysis||analyzeRoute(rt);
...
async function motorwayJunctionsAround(lat,lon,km=45){
  ...
  for(const url of endpoints){
    try{
      const r=await fetch(url,{method:'POST',headers:{'Content-Type':'text/plain'},body:q});
      if(!r.ok){last=new Error(`Overpass ${r.status}`);continue;}
      const d=await r.json();
      const out=(d.elements||[]).map(x=>({lat:x.lat,lon:x.lon,name:x.tags?.name||x.tags?.ref||'高速IC',ref:x.tags?.ref||''}));
      if(out.length)return out.sort((a,b)=>hav([lat,lon],[a.lat,a.lon])-hav([lat,lon],[b.lat,b.lon]));
    }catch(e){last=e;}
  }
  throw last||Error('高速道路IC情報を取得できませんでした');
}
function routeMotorwayRatio(rt){return motorwayScore(rt).ratio;}
function forcedHighwayAnalysis(rt){
  const a=analyzeRoute(rt);
  // 4点ルート [現在地, 高速IC, 高速IC, 目的地] の中央legは
  // 「高速ICから高速ICまで」を明示的に通した区間。OSRM step名だけで
  // 高速判定できない場合でも、0%と誤表示しないための補助情報として使う。
  const legs=Array.isArray(rt.legs)?rt.legs:[];
  if(legs.length>=3){
    const pre=Number(legs[0].distance)||0;
    const mid=Number(legs[1].distance)||0;
    const post=Number(legs[2].distance)||0;
    if(mid>5000){
      const startKm=pre/1000, endKm=(pre+mid)/1000;
      return {...a, forcedByIC:true, highwayDistance:Math.max(a.highwayDistance,mid),
        highwayRatio:Math.max(a.highwayRatio||0,mid/Math.max(1,pre+mid+post)),
        highwayStart:{label:'高速IC経由区間',km:startKm},
        highwayEnd:{label:'高速IC経由区間',km:endKm}};
    }
  }
  return a;
}
async function buildForcedHighwayRoutes(a,b){
  ...
  for(const pair of pairs.slice(0,10)){
    try{
      ...
      if(!rt)continue;
      const analysis=forcedHighwayAnalysis(rt);
      ...
      best.push({rt,ratio,analysis,oi:pair.oi,di:pair.di,signature});
    }catch(e){console.warn('IC経由ルート失敗',e);}
    if(best.length>=4)break;
  }
  best.sort((x,y)=>(y.ratio-x.ratio)||((x.rt.duration||Infinity)-(y.rt.duration||Infinity)));
  return best;
}
...
async function getRoutes(a,b){
  ...
  try{
    ...
    if(forced.length){
      const best=forced[0];
      const o=classifyRoute(best.rt,0,'highway',{ratio:best.ratio,analysis:best.analysis,roads:routeRoadSummary(best.rt,'highway')});
  ...
  // 2) 高速＋一般道：高速道路優先と同じ候補は採用しない。
  //    OSRMが1本しか返さない場合はValhallaの中間設定を試し、同一ルートの
  //    重複表示をできるだけ避ける。
  try{
    const rs=await osrm(a,b);
    const mixedCandidates=rs.filter(r=>!candidates.some(o=>routeSimilar(o.rt,r)));
    if(mixedCandidates.length){
      const mixed=mixedCandidates.sort((x,y)=>{const ax=analyzeRoute(x).highwayRatio??0,bx=analyzeRoute(y).highwayRatio??0;return Math.abs(ax-.45)-Math.abs(bx-.45)})[0];
  ...
  if(!candidates.some(o=>o.kind==='mixed')){
    try{
      const rt=await valhallaRoute(a,b,0.55);
      if(!candidates.some(o=>routeSimilar(o.rt,rt))){
        const o=classifyRoute(rt,candidates.length,'auto',{ratio:null,roads:'高速＋一般道'});
        o.name='高速＋一般道'; o.kind='mixed'; o.tag='別ルート';
        candidates.push(o); seenKinds.add('mixed');
      }
    }catch(e){console.warn('Valhalla mixed route failed',e);}
  }
  ...
  try{
    ...
    if(rs.length){
      const generalCandidates=rs.filter(r=>!candidates.some(o=>routeSimilar(o.rt,r)));
      const general=generalCandidates.sort((x,y)=>(analyzeRoute(x).highwayRatio??1)-(analyzeRoute(y).highwayRatio??1))[0] || rs[0];
  ...
  // 高速IC検索が一時失敗しても、OSRM alternatives の中から高速比率が最も高い候補を高速系として復旧。
  if(!candidates.some(o=>o.kind==='highway')){
    try{
      const rs=await osrm(a,b);
      const ranked=rs.map(r=>({r,a:analyzeRoute(r)})).sort((x,y)=>(y.a.highwayRatio??0)-(x.a.highwayRatio??0));
      const best=ranked.find(x=>(x.a.highwayRatio??0)>=0.25);
      if(best){
        const o=classifyRoute(best.r,candidates.length,'highway',{analysis:best.a,ratio:best.a.highwayRatio});
        o.name='高速道路優先';o.kind='highway';o.tag='おすすめ';candidates.push(o);seenKinds.add('highway');
      }
    }catch(e){console.warn('OSRM highway fallback failed',e);}
  }
...
async function overpassQuery(q){
  ...
  for(const url of endpoints){
    try{
      ...
      const r=await fetch(url,{method:'POST',headers:{'Content-Type':'text/plain'},body:q,signal:ac.signal});
      clearTimeout(timer);
      if(!r.ok) {last=new Error(`Overpass ${r.status}`);continue;}
      const d=await r.json();
...
const KNOWN_SAPA=[
  {name:'永福PA（上り）',type:'PA',lat:35.6681652,lon:139.6434317},
  {name:'大井PA（東行き）',type:'PA',lat:35.5925287,lon:139.7551013},
  {name:'海ほたるPA',type:'PA',lat:35.4638885,lon:139.8749598},
  {name:'君津PA（下り）',type:'PA',lat:35.283014,lon:139.9271192},
  {name:'君津PA（上り）',type:'PA',lat:35.2834743,lon:139.9260837}
];
function normalizeSpot(x){return {name:x.name,type:x.type||'PA',lat:+x.lat,lon:+x.lon};}
function highwayRangeAtKm(analysis,km){
  if(!analysis?.hasStepData)return false;
  const hit=(analysis.segments||[]).find(s=>s.isHighway && km>=s.fromKm-0.15 && km<=s.toKm+0.15);
  return !!hit;
}
function projectSpot(rt,sp,totalCum,analysis){
  const z=nearestOnRoute(rt,sp.lat,sp.lon);
  // 「ルートの近くにある」だけでは採用しない。SA・PAそのものが
  // 選択中ルートの高速道路区間上にある場合だけ表示する。
  if(z.d>1800)return null;
  const km=totalCum[z.i]/1000;
  if(!highwayRangeAtKm(analysis,km))return null;
  return {...sp,km,roadDist:z.d};
}
async function getSpots(rt){
  const cs=rt.geometry.coordinates;
  if(!Array.isArray(cs)||cs.length<2)throw Error('ルート形状を取得できませんでした');
  const totalCum=cum(cs), total=totalCum.at(-1)||0;
  const analysis=analyzeRoute(rt);
  // 高速区間を判定できないルートでは、近傍にあるだけの施設をSA・PAとして
  // 誤表示しない。一般道優先で高速SA/PAが出る問題をここで遮断する。
  if(!analysis.hasStepData || !(analysis.highwayDistance>1000))return [];
  const samples=[];
  // 5km間隔、各地点2.5km。巨大な一括Overpass検索を避ける。
  for(let d=0;d<=total;d+=5000){
    let i=0;while(i<totalCum.length-1&&totalCum[i+1]<d)i++;
    const c=cs[i]||cs.at(-1);samples.push([c[1],c[0]]);
  }
  const last=cs.at(-1);if(!samples.length||samples.at(-1)[0]!==last[1]||samples.at(-1)[1]!==last[0])samples.push([last[1],last[0]]);
  const clauses=samples.slice(0,28).map(([la,lo])=>`nwr["highway"~"^(services|rest_area)$"](around:2500,${la},${lo});`).join('');
  const q=`[out:json][timeout:30];(${clauses});out center tags;`;
  let elements=[];
  try{elements=await overpassQuery(q);}catch(e){console.warn('Overpass SA/PA failed',e);}
  const raw=[];
  const include=/(サービスエリア|パーキングエリア|PA|SA|ＰＡ|ＳＡ)/i;
  const exclude=/(道の駅|コインパーキング|駐車場|ロータリー|バス停|ガソリンスタンド|サービスステーション)/i;
  for(const x of elements){
    const la=x.lat??x.center?.lat,lo=x.lon??x.center?.lon;if(la==null||lo==null)continue;
    const t=x.tags||{},label=t['name:ja']||t.name||'';
    if(!label||exclude.test(label)||!include.test(label))continue;
    raw.push(normalizeSpot({name:label,type:/サービスエリア|SA|ＳＡ/i.test(label)&&!/パーキングエリア|PA|ＰＡ/i.test(label)?'SA':'PA',lat:la,lon:lo}));
  }
  // Overpassが空/一時障害でも、東京湾〜館山方面の主要SA/PAを最低限復旧。
  const candidates=[...raw,...KNOWN_SAPA];
  const out=[];
  for(const sp of candidates){
    const p=projectSpot(rt,sp,totalCum,analysis);if(!p)continue;
    if(p.km<1||p.km>total/1000-1)continue;
    if(current&&Number.isFinite(current.heading)&&current.speed>3&&angleDiff(current.heading,routeBearing(rt,nearestOnRoute(rt,p.lat,p.lon).i))>100)continue;
    if(out.some(v=>hav([v.lat,v.lon],[p.lat,p.lon])<500))continue;
    out.push(p);
  }
...
function draw(){
  ...
  const a=analyzeRoute(route),features=[];
  if(a.hasStepData){
    for(const leg of route.legs||[])for(const st of leg.steps||[]){
      if(st.geometry?.coordinates?.length>=2)features.push({type:'Feature',properties:{highway:stepIsHighway(st)},geometry:st.geometry});
    }
  }
  if(features.length){
    routeLayer=L.geoJSON({type:'FeatureCollection',features},{style:f=>({weight:6,opacity:.9,color:f.properties.highway?'#ff9f1c':'#3d8bfd'})}).addTo(map);
  }else{
    routeLayer=L.geoJSON(route.geometry,{style:{weight:5}}).addTo(map);
  }
  map.fitBounds(routeLayer.getBounds(),{padding:[15,15]});
  if(destMarker)map.removeLayer(destMarker);destMarker=L.marker([route.dest.lat,route.dest.lon]).addTo(map).bindPopup(`目的地: ${route.dest.label||'目的地'}`);
}
