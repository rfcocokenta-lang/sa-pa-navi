...
console.info('SA・PAナビ build v0.18');
...
async function getRoutes(a,b){
  ...
  try{
    ...
    if(rs.length){
      ...
      const mixed=mixedCandidates.sort((x,y)=>{const ax=analyzeRoute(x).highwayRatio??0,bx=analyzeRoute(y).highwayRatio??0;return Math.abs(ax-.45)-Math.abs(bx-.45)})[0] || rs[0];
...
function projectSpot(rt,sp,totalCum){const z=nearestOnRoute(rt,sp.lat,sp.lon);if(z.d>2500)return null;const km=totalCum[z.i]/1000;return {...sp,km,roadDist:z.d};}
async function getSpots(rt){
  ...
  for(const sp of candidates){
    const p=projectSpot(rt,sp,totalCum);if(!p)continue;
