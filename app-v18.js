...
async function getSpots(rt){
  ...
  // ルート全体を巨大な矩形で検索するとOverpassがタイムアウトしやすいため、
  // ルート上を約10km間隔でサンプリングして各地点の半径4kmだけ検索する。
  const totalCum=cum(cs), total=totalCum.at(-1)||0;
  const samples=[];
  for(let d=0;d<=total;d+=10000){
  ...
  const clauses=samples.slice(0,18).map(([la,lo])=>`nwr["highway"~"^(services|rest_area)$"](around:4500,${la},${lo});`).join('');
  const q=`[out:json][timeout:35];(${clauses});out center tags;`;
  let elements=[];
  try{elements=await overpassQuery(q);}catch(e){throw Error('SA・PAデータを取得できませんでした（Overpass混雑）');}
  const out=[],include=/(サービスエリア|パーキングエリア|\bSA\b|\bPA\b|SA・PA|ＳＡ|ＰＡ)/i,exclude=/(道の駅|コインパーキング|駐車場|VIP|ラウンジ|ロータリー|バス停|ガソリンスタンド|サービスステーション)/i;
  for(const x of elements){
    const la=x.lat??x.center?.lat,lo=x.lon??x.center?.lon;if(la==null||lo==null)continue;
    const t=x.tags||{},name=t.name||t['name:ja']||'',ja=t['name:ja']||name,label=ja||name;
    if(!label||exclude.test(label)||(!include.test(name)&&!include.test(ja)))continue;
    const z=nearestOnRoute(rt,la,lo);if(z.d>180)continue;
    const km=totalCum[z.i]/1000;if(km<1||km>total/1000-1)continue;
    if(current&&Number.isFinite(current.heading)&&current.speed>3&&angleDiff(current.heading,routeBearing(rt,z.i))>75)continue;
    if(!directionMatches(label,rt,z.i))continue;
    let type='PA';if(/サービスエリア|\bSA\b|ＳＡ/i.test(label)&&!/パーキングエリア|\bPA\b|ＰＡ/i.test(label))type='SA';
